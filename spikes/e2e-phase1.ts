...
for (const [k, v] of Object.entries({ OWNER_KEY, DELEGATE_KEY, ACCOUNT_ID, PACKAGE_ID })) {
...
const indexer = new MemForksIndexer({ treeId, suiClient: mem.suiClient, packageId: PACKAGE_ID });

let indexedCommits = 0;
indexer.on("commit", (node) => {
  indexedCommits++;
  console.log(`  [indexer] CommitCreated branch=${node.branch} isMerge=${node.isMerge}`);
});
indexer.on("branch", (ev) => {
  console.log(`  [indexer] BranchCreated branch=${ev.branch} from=${ev.fromBranch}`);
});
...
console.log(`Commits on-chain: ${indexedCommits > 0 ? "yes (indexed)" : "yes (not yet indexed)"}`);
console.log(`Recall works:    yes (distance ${results[0].distance.toFixed(4)})`);
console.log(`Indexer:         ${indexer.allBranches().length} branch(es) tracked`);
