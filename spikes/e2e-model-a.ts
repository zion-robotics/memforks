...
async function submitAttestation(opts: {
  ...
  if (result.effects?.status.status !== 'success') {
    throw new Error(
      `submit_attestation failed: ${result.effects?.status.error}`,
    );
  }
...
async function main() {
  ...
  console.log(
    '  (each commit waits for Walrus blob confirmation — ~30s total)',
  );
  const t0 = Date.now();
  ...
  ok(
    `commit 2  blob=${blob2.slice(0, 16)}...  parent=${blob1.slice(0, 10)}...`,
  );
  ...
  const localHeads = (
    mem as unknown as { heads: Map<string, { blobId: string }> }
  ).heads;
  assert(
    localHeads.get(branchName)?.blobId === blob2,
    'local head should be blob2',
  );
  assert(
    localHeads.get('main')?.blobId === mainBlob,
    'local head of main should be mainBlob',
  );
  ...
  results.forEach((r, i) =>
    console.log(
      `    [${i}] distance=${r.distance.toFixed(4)}  "${r.text.slice(0, 80)}..."`,
    ),
  );
  ...
  ok(
    `funded judges: ${judge0Addr.slice(0, 16)}...  ${judge1Addr.slice(0, 16)}...`,
  );

  // Grant delegates (required to attest on proposals).
  await mem.grantDelegate(judge0Addr, {
    perms: 0x1f,
    expiresEpoch: BigInt('18446744073709551615'),
  });
  await mem.grantDelegate(judge1Addr, {
    perms: 0x1f,
    expiresEpoch: BigInt('18446744073709551615'),
  });
  ...
  ok(
    `local    ${branchName} head: ${localHeads.get(branchName)?.blobId?.slice(0, 20) || '(empty)'}...`,
  );
  ok(
    `local    main head          : ${localHeads.get('main')?.blobId?.slice(0, 20) || '(empty)'}...`,
  );
  ...
  const propEvent = proposeTx.events?.find((e) =>
    e.type.includes('::resolver::MergeProposed'),
  );
  const proposalId: string =
    (propEvent?.parsedJson as { proposal_id?: string } | undefined)
      ?.proposal_id ?? '';
  ...
  assert(
    status === 'finalized',
    `proposal status should be finalized, got: ${status}`,
  );
  ok(`proposal status: ${status}`);
  ok(
    `resolved_blob_id: ${proposal.resolved_memwal_blob_id ?? '(not in proposal fields)'}`,
  );
  ...
  const finalizedEvent = finalizeTx.events?.find((e) =>
    e.type.includes('::resolver::MergeFinalized'),
  );
  const mergeCommitId: string =
    (finalizedEvent?.parsedJson as { merge_commit_id?: string } | undefined)
      ?.merge_commit_id ?? '';
  assert(
    mergeCommitId.length > 0,
    'merge_commit_id must be present in MergeFinalized event',
  );
  ...
  ok(
    `anchor.parents = [${(anchor.parents as string[]).map((p) => p.slice(0, 10) + '...').join(', ')}]`,
  );
  assert(
  ...
  assert(
    mainHead === blob2,
    `main head should be blob2 (${blob2.slice(0, 16)}...), got: ${mainHead.slice(0, 16)}...`,
  );
  ...
  step(
    10,
    TOTAL_STEPS,
    'Indexer: poll for BranchCreated + MergeFinalized events',
  );
  ...
  indexer.on(
    'merge_finalized',
    (ev: { intoBranch: string; mergeCommitId: string }) => {
      mergeEvents++;
      console.log(
        `    [indexer] MergeFinalized: into=${ev.intoBranch} anchor=${ev.mergeCommitId.slice(0, 12)}...`,
      );
    },
  );
  ...
  assert(
    branchEvents > 0,
    'indexer should have seen at least one BranchCreated event',
  );

  if (mergeEvents === 0) {
    // Direct fallback: query for the MergeFinalized event we know was emitted.
    console.log(
      '  (indexer missed it in window — querying directly via RPC fallback)',
    );
    ...
    const found = evResult.data.some(
      (e) =>
        (e.parsedJson as Record<string, unknown>)?.['merge_commit_id'] ===
        mergeCommitId,
    );
    assert(
      found,
      `MergeFinalized event for ${mergeCommitId.slice(0, 16)} not found on-chain`,
    );
  ...
  try {
    ...
    const cfgR = await fetch(`${UI_ORIGIN}/api/config`, {
    ...
    if (srvTree && srvTree !== TREE_ID) {
      console.warn(
        `  ⚠  Server is configured for tree ${srvTree.slice(0, 16)}...`,
      );
      console.warn(
        `     Test used tree                 ${TREE_ID.slice(0, 16)}...`,
      );
      console.warn(
        `     Namespaces differ — run: memfork init (or set MEMFORKS_TREE_ID in .memfork.json)`,
      );
      ...
      if (r.ok) {
        ...
        assert(
          count >= 2,
          `/api/history should return ≥2 commits for ${branchName}, got ${count}`,
        );
      } else {
    ...
    console.warn(
      `  ⚠  /api/history unreachable (start: memfork ui). Skipping.`,
    );
  }
  ...
  console.log(
    `  Commits (blobs) : ${blob1.slice(0, 10)}...  ${blob2.slice(0, 10)}...  ${mainBlob.slice(0, 10)}...`,
  );
